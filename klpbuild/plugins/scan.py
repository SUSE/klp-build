...
from klpbuild.klplib.bugzilla import get_pending_bugs, get_bug_data, is_bug_dropped, get_bug_dep, get_bug_summary
...
def run(cve, conf, lp_filter, download):
    ...
    if not cve:
        # Fetch now to avoid doing it during each scan() job.
        get_supported_codestreams()
        scan_bugzilla()
        return
...
def scan_bugzilla():
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        for b in bugs:
            d = get_bug_dep(b, deps)
            cve, system, cvss, level, prio  = get_bug_data(b, d)
            if not cve:
                continue
            job = executor.submit(scan_job, b, d, cve)
    ...
    logging.info(tabulate(table, headers=["ID", "CVE", "SUBSYSTEM", "CVSS", "PRIORITY",
                                          "CLASSIFICATION", "STATUS", "AFFECTED"]))


def scan_job(bug, dep, cve):
    affected = "No"
    status = "Not-Fixed"

    patches, upstream, patched_cs, affected_cs = scan(cve, None, None, False)
