...
class IBS():
    ...
    def prepare_tests(self):
        ...
        for arch in ARCHS:
            ...
            for cs in filter_codestreams(self.lp_filter, "", get_codestreams_dict()):
                ...
                rpm_dir = Path(cs.get_cpp_dir(), arch, "rpm")
    ...
    def delete_rpms(self, cs):
        try:
            for arch in cs.archs:
                shutil.rmtree(Path(cs.get_cpp_dir(), arch, "rpm"), ignore_errors=True)
        except KeyError:
            pass
        ...
        for result in self.get_projects():
            ...
            for arch in archs:
                ...
                for rpm in ret.xpath("binary/@filename"):
                    ...
                    dest = Path(cs.get_cpp_dir(), str(arch), "rpm")
                    dest.mkdir(exist_ok=True, parents=True)
    ...
    def create_lp_package(self, i, cs):
        ...
        prj_path = Path(cs.get_cpp_dir(), "checkout")
        if prj_path.exists():
            shutil.rmtree(prj_path)

        code_path = Path(cs.get_cpp_dir(), "code")
        if code_path.exists():
